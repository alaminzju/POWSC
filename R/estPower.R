...
PowerEst = function(fdr, alpha, Zg, Zg2, xgr){
    ...
    list(TD=TD,FD=FD,alpha.nominal=alpha,
...
Power_Cont = function(DErslt, simData, alpha = 0.1, delta = 0.5){
    fdrvec = DErslt$cont$fdr
    lfc = simData$lfc
    ngenes = length(simData$ix.DE2)
    DEid = simData$ix.DE2
    Zg = Zg2 = rep(0, ngenes)
    Zg[DEid] = 1
...
Power_Disc = function(DErslt, simData, alpha = 0.1, delta = 0.1){
    fdrvec = DErslt$disc$fdr
    pi.df = simData$pi.df
    ngenes = length(simData$ix.DE1)
    DEid = simData$ix.DE1
    Zg = Zg2 = rep(0, ngenes)
    Zg[DEid] = 1
